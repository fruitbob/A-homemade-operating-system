...
unsigned char mback[17][17];
unsigned char cmdback[512*768];
...
void init_window(struct BOOTINFO *binfo)
{
	...
	os_draw_square(binfo->vram,binfo->scrnx,0,0,14,binfo->scrnx,binfo->scrny);
	os_draw_square(binfo->vram,binfo->scrnx,0,binfo->scrny-17,COL8_C6C6C6,binfo->scrnx,binfo->scrny);
	os_draw_square(binfo->vram,binfo->scrnx,0,binfo->scrny-17,COL8_00FFFF,39,binfo->scrny);
	os_printfs(binfo->vram,binfo->scrnx,0,binfo->scrny-17,0,COL8_00FFFF,"BASIC ",5);
	os_draw_square(binfo->vram,binfo->scrnx,binfo->scrnx-45,binfo->scrny-17,0,1,17);
	os_outchar16(binfo->vram,binfo->scrnx,binfo->scrnx-43,binfo->scrny-17,0,'t');
	os_outchar16(binfo->vram,binfo->scrnx,binfo->scrnx-35,binfo->scrny-17,0,'i');
	os_outchar16(binfo->vram,binfo->scrnx,binfo->scrnx-27,binfo->scrny-17,0,'m');
	os_outchar16(binfo->vram,binfo->scrnx,binfo->scrnx-19,binfo->scrny-17,0,'e');
	//os_printfs(binfo->vram,binfo->scrnx,binfo->scrnx-43,binfo->scrny-17,0,1," time",5);
	//os_printfs(binfo->vram, binfo->scrnx, 0, binfo->scrny - 17, 0, COL8_00FFFF, "BASIC ", 5);
	os_printfs(binfo->vram, binfo->scrnx, binfo->scrnx / 9 * 2 + 15, binfo->scrny / 5 * 4 - 16 * 4, 0, COL8_00FFFF, "***                Making information!!!                 ***",60);
	os_printfs(binfo->vram, binfo->scrnx, binfo->scrnx / 9 * 2 + 15, binfo->scrny / 5 * 4 - 16*2+5, 0, COL8_00FFFF, "***                  Made by Zhao Yubo                   ***", 60);
	os_printfs(binfo->vram, binfo->scrnx, binfo->scrnx / 9 * 2 + 15, binfo->scrny / 5 * 4+5  , 0, COL8_00FFFF, "***        Information Security Class 4, 20162400        ***", 60);
	os_printfs(binfo->vram, binfo->scrnx, binfo->scrnx / 9 * 2+15, binfo->scrny / 5 * 4 + 5+16*2, 0, COL8_00FFFF, "***  School of Computer Science and Engineering in TJUT  ***", 60);
}
...
void os_printfs(unsigned char *vram,int sc_x,int x0,int y0,int cl,int bc,char *s,int l)
{
	...
	int i;
	os_draw_square(vram,sc_x,x0,y0,bc,l*8,16);
	for(i=0;i<l;i++)
	{
		
		os_outchar16(vram,sc_x,x0,y0,cl,*s);
		x0+=8; 
		s++;
	 } 
...
void init_mouse16(const struct BOOTINFO *binfo,int p_x,int p_y,int s_x) 
{
	...
	for(y=0;y<16;y++)
	{
		for(x=0;x<16;x++)
		{
			mback[y][x]=binfo->vram[(p_y+y)*binfo->scrnx+x+p_x];
			//if(mouse[y][x]=='*') vram[(p_y+y)*320+x+p_x]=vram[p_mouse];
			if(mouse[y][x]=='.') binfo->vram[(p_y+y)*binfo->scrnx+x+p_x]=(char)4;
		}
	}
	return;
}
void change_back(const struct BOOTINFO *binfo,int p_x,int p_y,int s_x) 
{
	...
	for(y=0;y<16;y++)
	{
		for(x=0;x<16;x++)
		{
			binfo->vram[(p_y+y)*binfo->scrnx+x+p_x]=mback[y][x];
		}
	}
	return;
}
...
{
	int max = binfo->scrnx*(binfo->scrny / 5 );
	if (max >= 512 * 768)
	{
		os_printfs(binfo->vram, binfo->scrnx, 0, binfo->scrny - 34, 0, COL8_00FFFF, "error out of buf ", 20);
		//return;
	}
	int i;
	for (i = 0; i <  max; i++)
	{
		cmdback[i] = binfo->vram[binfo->scrnx*(binfo->scrny / 5*4-1) + i];
	}
...
void dis_cmd(struct BOOTINFO *binfo)
{
	...
	for (i = 0; i < max; i++)
	{
		binfo->vram[binfo->scrnx*(binfo->scrny / 5 * 4 - 1) + i] = cmdback[i];
	}
	os_printfs(binfo->vram, binfo->scrnx, 0, binfo->scrny - 17, 0, COL8_00FFFF, "BASIC ", 5);
	return;
}
...
{
	os_draw_square(binfo->vram, binfo->scrnx, cur_x, cur_y, cur_c, 2, 16);
}
void cur_out(struct BOOTINFO *binfo, int *cur_y)
{
	int i;
	if (*cur_y >= binfo->scrny - 34)
	{
		int max = binfo->scrnx*(binfo->scrny / 5) - binfo->scrnx * 16;
		for (i = 0; i < max; i++)
		{
			binfo->vram[binfo->scrnx*(binfo->scrny / 5 * 4 - 1) + i] = binfo->vram[binfo->scrnx*(binfo->scrny / 5 * 4 - 1) + i + binfo->scrnx * 18];
		}
		*cur_y -= 18;
		os_draw_square(binfo->vram, binfo->scrnx, 0, *cur_y, 7, binfo->scrnx, binfo->scrny - 16 - *cur_y);
		//init_cmd(binfo);
		//cur_x = 18;
